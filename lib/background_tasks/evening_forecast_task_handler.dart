...
class EveningForecastTaskHandler {
  ...
  Future<bool> execute() async {
    ...
    final bool isEnabled =
        prefs.getBool(NotificationSettingsRepository.eveningForecastEnabledKey) ??
            false;

    if (!isEnabled) {
      dev.log("Task skipped: Evening forecast is disabled by the user.",
          name: logName);
      return true;
    }
    ...
    try {
      final adminDivision = prefs.getString('last_known_admin_division');

      if (adminDivision == null) {
        dev.log("Task skipped: No last known location has been saved yet.", name: logName);
        return true;
      }
      ...
      if (tomorrowsForecasts.isEmpty) {
        dev.log("No forecast entries found for tomorrow in $adminDivision.",
            name: logName);
        return true;
      }
      ...
      dev.log("Evening forecast notification sent for $adminDivision.",
          name: logName);
